...
// Errors.
var errBcachefsNoQuota = fmt.Errorf("Quotas disabled on filesystem")
var errBcachefsNoQGroup = fmt.Errorf("Unable to find quota group")
...
func (d *bcachefs) getMountOptions() string {
	// Allow overriding the default options.
	if d.config["bcachefs.mount_options"] != "" {
		return d.config["bcachefs.mount_options"]
	}

	return "user_subvol_rm_allowed"
}

// TODO: Current strategy for btrfs doesn't work,
// looks like something to do with ioctl does
// try sudo bcachefs subvolume delete tester.c  on
// anything but a subvolume
// will get error, except for bcachefs subvolumes
// https://bcachefs.org/Snapshots/#:~:text=Subvolumes%20in%20bcachefs%20are%20just%20fancy%20directories.%20We,fields%20for%20root%20inode%20number%20and%20snapshot%20ID.
// Check above, scroll to subvolumes for listing subvolumes and checking
// if something is subvolume
func (d *bcachefs) isSubvolume(path string) bool {
	// Stat the path.
	fs := unix.Stat_t{}
...
// TODO: Make decision as to whether this should exist or not,
// bcache can't list subvolume
func (d *bcachefs) hasSubvolumes(path string) (bool, error) {
...
func (d *bcachefs) getSubvolumes(path string) ([]string, error) {
...
func (d *bcachefs) snapshotSubvolume(path string, dest string, recursion bool) (revert.Hook, error) {
	revert := revert.New()
	defer revert.Fail()

	// Single subvolume creation.
	snapshot := func(path string, dest string) error {
		_, err := subprocess.RunCommand("bcachefs", "subvolume", "snapshot", path, dest)
		if err != nil {
			return err
		}
...
func (d *bcachefs) deleteSubvolume(rootPath string, recursion bool) error {
	...
	destroy := func(path string) error {
		...
		_, err = subprocess.RunCommand("bcachefs", "subvolume", "delete", path)
...
func (d *bcachefs) getQGroup(path string) (string, int64, error) {
	// Try to get the qgroup details.
	// TODO: No such thing as qgroup show in bcachefs
	output, err := subprocess.RunCommand("bcachefs", "qgroup", "show", "-e", "-f", "--raw", path)
	if err != nil {
		return "", -1, errBcachefsNoQuota
	}
	...
	if qgroup == "" {
		return "", -1, errBcachefsNoQGroup
	}

	return qgroup, usage, nil
}

func (d *bcachefs) sendSubvolume(path string, parent string, conn io.ReadWriteCloser, tracker *ioprogress.ProgressTracker) error {
	...
	cmd := exec.Command("bcachefs", args...)
	...
	if err != nil {
		logger.Errorf("Problem reading bcachefs send stderr: %s", err)
	}

	err = cmd.Wait()
	if err != nil {
		return fmt.Errorf("Bcachefs send failed: %w (%s)", err, string(output))
	}

	return nil
}

// TODO: Investigate, bcachefs does have ro for mount, not sure
// for subvolumes
// setSubvolumeReadonlyProperty sets the readonly property on the subvolume to true or false.
func (d *bcachefs) setSubvolumeReadonlyProperty(path string, readonly bool) error {
...
func (d *bcachefs) getSubvolumesMetaData(vol Volume) ([]BCACHEFSSubVolume, error) {
	var subVols []BCACHEFSSubVolume
	...
	subVols = append(subVols, BCACHEFSSubVolume{
		Snapshot: snapName,
...
func (d *bcachefs) getSubVolumeReceivedUUID(vol Volume) (string, error) {
	stdout := strings.Builder{}

	poolMountPath := GetPoolMountPath(vol.pool)

	// TODO: Investigate if bcachefs can do list subvolumes
...
// Note: This is used by both migration and backup subsystems so do not modify without considering both!
type BCACHEFSMetaDataHeader struct {
	Subvolumes []BCACHEFSSubVolume `json:"subvolumes" yaml:"subvolumes"` // Sub volumes inside the volume (including the top level ones).
}
...
func (d *bcachefs) restorationHeader(vol Volume, snapshots []string) (*BCACHEFSMetaDataHeader, error) {
	var migrationHeader BCACHEFSMetaDataHeader
...
func (d *bcachefs) loadOptimizedBackupHeader(r io.ReadSeeker, mountPath string) (*BCACHEFSMetaDataHeader, error) {
	header := BCACHEFSMetaDataHeader{}
...
func (d *bcachefs) receiveSubVolume(r io.Reader, receivePath string, tracker *ioprogress.ProgressTracker) (string, error) {
